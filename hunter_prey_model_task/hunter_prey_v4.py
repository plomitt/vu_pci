...
class LVConfig(Config):
    ...
    rabbit_perception_radius: int = 100 # INCREASED: Rabbits can see further to find grass
    ...
    fox_rabbit_energy_gain: float = 10.0   # Energy gained by fox when eating a rabbit
    ...
    rabbit_grass_energy_gain: float = 5.0 # Energy gained by rabbit when eating grass
    rabbit_starvation_threshold: float = 0.0 # Energy level at which a rabbit dies

    # NEW: Grass Parameters
    grass_spawn_locations_count: int = 200 # INCREASED: More grass patches to make visibility easier
    grass_re_grow_time: int = 500         # INCREASED: Ticks for grass to regrow after being eaten (for easier observation)
...
class Grass(Agent[LVConfig]):
    """
    Represents a Grass agent.
    Grass is static and can be consumed by rabbits. It regrows after a certain time.
    """
    is_eaten: bool = False # State: True if eaten, False if available
    re_grow_timer: int = 0 # Timer for regrowth
    _original_image_surface: pg.Surface = None # Store the actual surface to draw

    def on_spawn(self):
        """Initializes grass. Grass does not move."""
        self.move = Vector2(0, 0) # Grass is static
        self.re_grow_timer = 0
        self.is_eaten = False
        
        # Store the first image provided to the Agent's constructor as the original
        if self._images and len(self._images) > 0:
            self._original_image_surface = self._images[0]
        else:
            # Fallback if no image provided (should be handled by run_experiment)
            # This dummy image will be used if grass.png fails to load.
            self._original_image_surface = pg.Surface((10, 10), pg.SRCALPHA)
            pg.draw.circle(self._original_image_surface, (0, 100, 0), (5, 5), 5)
            print(f"{YELLOW}Warning: Grass agent spawned without an image. Using dummy image.{RESET}")

    def change_position(self):
        """
        Grass regrowth logic.
        """
        prev_is_eaten = self.is_eaten # Store previous state for console message
        if self.is_eaten:
            self.re_grow_timer += 1
            if self.re_grow_timer >= self.config.grass_re_grow_time:
                self.is_eaten = False
                self.re_grow_timer = 0
                if prev_is_eaten: # Only print if it actually regrew from an eaten state
                    print(f"{GREEN}Grass at {self.pos} just regrew!{RESET}")
        # The draw method will handle visibility based on self.is_eaten

    # Override the draw method to control visibility
    def draw(self, surface: pg.Surface):
        """
        Draws the grass agent onto the given surface if it's not eaten.
        """
        # print(f"DEBUG: Grass {self.id} drawing - is_eaten: {self.is_eaten}, pos: {self.pos}") # DEBUG PRINT
        if not self.is_eaten and self._original_image_surface:
            # Draw the grass image at its position
            surface.blit(self._original_image_surface, self.rect)
        # If is_eaten is True, nothing is drawn, making it invisible.
...
class Rabbit(Agent[LVConfig]):
    ...
    def on_spawn(self):
        ...
        self.config.radius = self.config.rabbit_perception_radius # Set perception radius for this agent type

    def change_position(self):
        """
        Rabbit movement, energy decay, eating grass, reproduction, and death logic.
        """
        self.config.radius = self.config.rabbit_perception_radius # Ensure correct radius is active

        # --- Energy Decay for Rabbit ---
        if self.config.energy_enabled:
            self.energy -= self.config.rabbit_energy_decay_rate
            if self.energy <= self.config.rabbit_starvation_threshold:
                self.kill() # Rabbit dies of starvation
                return # Stop further processing for this dead rabbit
        ...
        if self.config.energy_enabled:
            ...
            for neighbor, distance in self.in_proximity_accuracy():
                if isinstance(neighbor, Grass) and not neighbor.is_eaten: # Only target available grass
                    if distance < min_dist_grass:
                        min_dist_grass = distance
                        target_grass = neighbor
            
            # INCREASED Eating distance for testing
            if target_grass and self.pos.distance_to(target_grass.pos) < 30: # Close enough to eat grass
                target_grass.is_eaten = True
                target_grass.re_grow_timer = 0
                self.energy += self.config.rabbit_grass_energy_gain # Replenish energy
                # Ensure energy doesn't exceed initial energy (optional, for balance)
                self.energy = min(self.energy, self.config.rabbit_initial_energy * 2) # Cap at twice initial energy
                print(f"Rabbit {self.id} ate grass at {target_grass.pos}! Grass now eaten: {target_grass.is_eaten}. Energy: {self.energy:.2f}") # DEBUG Print

            # Move towards closest grass if energy is low, otherwise random walk
            if self.config.energy_enabled and self.energy < (self.config.rabbit_initial_energy / 2): # If energy is low
                if target_grass:
                    direction_to_grass = (target_grass.pos - self.pos).normalize()
                    self.move = direction_to_grass * self.config.rabbit_movement_speed
                else: # No grass in sight, random walk
            ...
            else: # Normal random walk if energy is sufficient
                if self.shared.prng_move.random() < 0.1:
                    self.move = self.move.rotate(self.shared.prng_move.uniform(-10, 10))
                    self.move.normalize_ip()
                    self.move *= self.config.rabbit_movement_speed
...
class Fox(Agent[LVConfig]):
    ...
    def on_spawn(self):
        ...
        self.config.radius = self.config.fox_perception_radius # Set perception radius for this agent type

    def change_position(self):
        """
        Fox movement, hunting, reproduction (by eating), and death logic.
        """
        self.config.radius = self.config.fox_perception_radius # Ensure correct radius is active
        ...
        if target_rabbit:
            ...
            if self.pos.distance_to(target_rabbit.pos) < 10: # Close enough to eat
                ...
                if self.config.energy_enabled:
                    self.energy += self.config.fox_rabbit_energy_gain # Use RENAMED parameter
                    # Ensure energy doesn't exceed initial energy (optional, for balance)
                    self.energy = min(self.energy, self.config.fox_initial_energy * 2) # Cap at twice initial energy
        else:
            if self.shared.prng_move.random() < 0.1:
                self.move = self.move.rotate(self.shared.prng_move.uniform(-10, 10))
                self.move.normalize_ip()
...
def spawn_agent_safely(agent_class: Type[Agent], images_list: List[pg.Surface], num_to_spawn: int, simulation_instance: Simulation):
    ...
    # Create a dummy agent to check for collisions without adding to simulation yet
    ...
    while count < num_to_spawn:
        ...
        while attempts < max_spawn_attempts_per_agent:
            ...
            is_colliding = False
            for obstacle in simulation_instance._obstacles: # _obstacles stores existing agents for collision checks
...
def run_experiment(
    ...
    grass_population_history = [] # NEW: To store grass population per tick
    fox_energy_history = [] # New: To store average fox energy per tick
    rabbit_energy_history = [] # NEW: To store average rabbit energy per tick
    ...
    # Load agent images and create dummy if not found
    ...
    grass_image_path = os.path.join(script_dir, "images", "grass.png") # NEW: Grass image path
    ...
    try:
        ...
        loaded_grass_image = pg.image.load(grass_image_path).convert_alpha() # NEW: Load grass image
        ...
        dummy_grass_surf = pg.Surface((10, 10), pg.SRCALPHA) # Dark green circle for grass
        pg.draw.circle(dummy_grass_surf, (0, 100, 0), (5, 5), 5) 
    ...
    # NEW: Spawn static grass patches if energy system is enabled
    if current_config.energy_enabled:
        spawn_agent_safely(Grass, grass_images_list, current_config.grass_spawn_locations_count, sim)
    ...
    def custom_tick_for_run(self): # 'self' refers to the Simulation instance
        ...
        # NEW: Collect current available grass population
        current_available_grass = len([agent for agent in self._agents if isinstance(agent, Grass) and not agent.is_eaten])
        ...
        grass_population_history.append(current_available_grass) # NEW: Append grass population
        ...
        if self.config.energy_enabled:
            ...
            # NEW: Collect average rabbit energy if enabled
    ...
    df_populations = pd.DataFrame({
        ...
        'Available_Grass': grass_population_history # NEW: Include grass population
    })
    ...
    # NEW: Save Grass Population plot to PNG (separate graph per run)
    if grass_population_history:
        plt.figure(figsize=(12, 7))
        plt.plot(np.arange(len(grass_population_history)), grass_population_history, label='Available Grass', color='darkgreen', linewidth=2)
        plt.title(f'Available Grass Population for {run_name}', fontsize=16)
        plt.xlabel('Time (Simulation Ticks)', fontsize=12)
        plt.ylabel('Grass Count', fontsize=12)
        plt.legend(loc='upper right', fontsize=10)
        plt.grid(True, linestyle='--', alpha=0.6)
        
        if grass_population_history:
            plt.ylim(0, max(grass_population_history) * 1.1 + 1)
        else:
            plt.ylim(0, current_config.grass_spawn_locations_count * 1.1 + 1) # If no history, base on initial count

        plt.tight_layout()
        grass_plot_path = os.path.join(run_output_dir, f"{run_name}_grass_population_dynamics.png")
        plt.savefig(grass_plot_path)
        plt.close()
        print(f"Grass population plot saved to: {grass_plot_path}")
    ...
    if current_config.energy_enabled:
        ...
        # NEW: Save Average Rabbit Energy data and plot (if energy is enabled)
    ...
    return {
        ...
        'rabbit_energy_history': rabbit_energy_history, # NEW: Include rabbit energy history
        ...
        # grass_population_history is only saved per run, not aggregated
    }
...
def analyze_category_results(results_list: List[dict], category_output_dir: str, category_name: str):
    ...
    all_rabbit_energy_histories = [] # NEW: For time series averaging of rabbit energy
    all_rabbit_population_histories = [] # For time series averaging of rabbit population
    all_fox_population_histories = [] # For time series averaging of fox population
    # No all_grass_population_histories here as per user request to only plot per run
    ...
    for result in results_list:
        ...
        # NEW: Collect rabbit energy history
    ...
    # NEW: Plot Averaged Rabbit Energy Over Time (if applicable)
    if all_rabbit_energy_histories:
        max_len_energy_rabbit = max(len(h) for h in all_rabbit_energy_histories)
        
        padded_rabbit_energy_histories = []
        for history in all_rabbit_energy_histories:
            if history:
                padded_rabbit_energy_histories.append(np.pad(history, (0, max_len_energy_rabbit - len(history)), 'edge')) 
            else:
                padded_rabbit_energy_histories.append(np.pad([], (0, max_len_energy_rabbit), 'constant', constant_values=0))
...
param_abbreviations = {
    ...
    'fox_rabbit_energy_gain': 'freg', # UPDATED abbreviation for fox's rabbit energy gain
    'fox_starvation_threshold': 'fst',
    # NEW Rabbit Energy & Grass Params
    'rabbit_initial_energy': 'rie',
    'rabbit_energy_decay_rate': 'redr',
    'rabbit_grass_energy_gain': 'rgeg', # UPDATED abbreviation for rabbit's grass energy gain
    'rabbit_starvation_threshold': 'rst',
    'grass_spawn_locations_count': 'gslc',
    'grass_re_grow_time': 'grgt'
}
...
def compare_scenarios(base_results_dir: str, scenario_names: List[str]):
    ...
    scenario_fox_energy_data = {} # Renamed for clarity
    for name in scenario_names:
    ...
    # NEW: Load averaged rabbit energy time series for each scenario
    ...
    # --- NEW Plot 4: Compared Average Fox and Rabbit Energy Dynamics ---
    plt.figure(figsize=(14, 8))
    has_energy_data = False
    for name in scenario_names:
        fox_df = scenario_fox_energy_data.get(name)
        rabbit_df = scenario_rabbit_energy_data.get(name)

        if fox_df is not None and not fox_df.empty:
            plt.plot(fox_df['Tick'], fox_df['Average_Fox_Energy'], label=f'{name.replace("_", " ").title()} - Fox Energy', linestyle='-', linewidth=2, color='orange')
            has_energy_data = True
        if rabbit_df is not None and not rabbit_df.empty:
            plt.plot(rabbit_df['Tick'], rabbit_df['Average_Rabbit_Energy'], label=f'{name.replace("_", " ").title()} - Rabbit Energy', linestyle='--', linewidth=2, color='purple')
            has_energy_data = True
    
    if has_energy_data:
        plt.title('Compared Averaged Energy Dynamics Over Time', fontsize=16)
        plt.xlabel('Time (Simulation Ticks)', fontsize=12)
        plt.ylabel('Average Energy', fontsize=12)
        plt.legend(loc='upper right', fontsize=10)
        plt.grid(True, linestyle='--', alpha=0.6)
        ...
        # Calculate overall max for y-limit
        max_energy_overall = 0
        for df in scenario_fox_energy_data.values():
            if df is not None and not df.empty and 'Average_Fox_Energy' in df.columns:
                max_energy_overall = max(max_energy_overall, df['Average_Fox_Energy'].max())
        for df in scenario_rabbit_energy_data.values():
            if df is not None and not df.empty and 'Average_Rabbit_Energy' in df.columns:
                max_energy_overall = max(max_energy_overall, df['Average_Rabbit_Energy'].max())
        
        plt.ylim(0, max_energy_overall * 1.1 + 1)

        plt.tight_layout()
        plot_path = os.path.join(base_results_dir, "compared_average_energy_dynamics.png")
        plt.savefig(plot_path)
        plt.close()
        print(f"Compared average energy dynamics plot saved to: {plot_path}")
    else:
        print(f"{YELLOW}No energy data available for comparison plot.{RESET}")
    ...
    # NEW: For energy dynamics (fox and rabbit)
    combined_energy_df = pd.DataFrame()
    for name in scenario_names:
        fox_df = scenario_fox_energy_data.get(name)
        rabbit_df = scenario_rabbit_energy_data.get(name)

        # Merge on 'Tick' or create if empty
        if combined_energy_df.empty and (fox_df is not None and not fox_df.empty or rabbit_df is not None and not rabbit_df.empty):
             # Prefer fox_df for tick range if available, else rabbit_df
            if fox_df is not None and not fox_df.empty:
                combined_energy_df['Tick'] = fox_df['Tick']
            elif rabbit_df is not None and not rabbit_df.empty:
                combined_energy_df['Tick'] = rabbit_df['Tick']

        if fox_df is not None and not fox_df.empty:
            combined_energy_df[f'{name}_Average_Fox_Energy'] = fox_df['Average_Fox_Energy']
        if rabbit_df is not None and not rabbit_df.empty:
            combined_energy_df[f'{name}_Average_Rabbit_Energy'] = rabbit_df['Average_Rabbit_Energy']
    
    if not combined_energy_df.empty:
        combined_energy_csv_path = os.path.join(base_results_dir, "compared_average_energy_time_series.csv")
        combined_energy_df.to_csv(combined_energy_csv_path, index=False)
        print(f"Combined average energy time series data saved to: {combined_energy_csv_path}")
...
if __name__ == "__main__":
    ...
    # This scenario acts as a baseline where neither foxes nor rabbits use an energy system.
    ...
    fixed_energy_free_params = {
        'energy_enabled': False # Explicitly disable energy for this scenario
    }
    ...
    # This scenario enables the energy system for both foxes and rabbits,
    # and includes parameters specific to their energy mechanics and grass.
    energy_enabled_params_to_vary = {
        'fox_initial_energy': [100.0],
        'fox_energy_decay_rate': [0.5],
        'fox_rabbit_energy_gain': [10.0], # UPDATED parameter name
        'fox_starvation_threshold': [0.0],
        'rabbit_initial_energy': [50.0], # NEW: Rabbit initial energy
        'rabbit_energy_decay_rate': [0.2], # NEW: Rabbit energy decay
        'rabbit_grass_energy_gain': [5.0], # UPDATED parameter name
        'rabbit_starvation_threshold': [0.0], # NEW: Rabbit starvation threshold
        'grass_spawn_locations_count': [200], # NEW: Number of grass patches (increased for visibility)
        'grass_re_grow_time': [500], # NEW: Grass regrowth time (increased for observation)
        'rabbit_movement_speed': [2.0],
        'fox_movement_speed': [3.0],
        'rabbit_perception_radius': [100], # INCREASED
        'fox_perception_radius': [70]
    }
    fixed_energy_enabled_params = {
        'energy_enabled': True # Explicitly enable energy for this scenario
    }
